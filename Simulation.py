...
call_states = {"Connected": 0, "Reneged": 0}  #Track call outcomes
...
def call_process_1(env, caseid_queue, event_log):
    global total_wait_time, total_calls, total_service_time
    ...
    print(f"curr queue length: {curr_queue_length}")
    if (curr_queue_length > reneg_queue_length_1):
        renege(0, caseid_queue)
        return
    ...
    with available_sp.request() as req:
        ...
        wait_times.append(wait_time)
        call_states["Connected"] += 1
        print(f"Call {caseid} connected to sp after waiting {wait_time:.2f} minutes")
        ...
        # update metrics
        service_time = env.now - wait_time
        total_service_time += service_time
        service_times.append(service_time)

        queue_length_on_leaving_service = len(caseid_queue)
...
def call_process_2(env, caseid_queue, event_log):
    global total_wait_time, total_calls, total_reneged_calls, total_escalated_calls, total_service_time
    ...
    with available_managers.request() as req_2:
        ...
        print("End_escalated_service caseid =",caseid,'time = ',env.now,'escalated_queue_length =',queue_length_on_leaving_service)
...
def renege(id, queue) :
    ...
    print("reneged process 1 =",caseid,'time = ',env.now,'initial_queue_length =',len(queue) + 1)
    ...
    while True:  
        ...
        if len(caseid_queue_1) > 0:
            ...
            if random.random() < 0.15:
                ...
                activity = 'service request escalated'
                if len(caseid_queue_2) < reneg_queue_length_2:
                    env.process(event_log_append(env, caseid, time, activity, event_log)) 
